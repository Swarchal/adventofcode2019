...
proc parseInput(path: string): seq[int] =
  return path.readFile.splitLines.map(parseInt)


func countFuel(mass: int): int =
  return mass.floorDiv(3) - 2


let
  input = parseInput(paramStr(1))


echo input.map(countFuel).sum()
...
func countFuelInclus(mass: int): int =
  var
    fuel_required = countFuel(mass)
    mass_fuel = fuel_required
  while countFuel(mass_fuel) > 0:
    mass_fuel = countFuel(mass_fuel)
    fuel_required += mass_fuel
  return fuel_required


echo input.map(countFuelInclus).sum()